...
- **PR #203** (`claude/wage-calculator-3nsleq`, base #198): wage calculator was broken since the WordPress migration (all input/button/script tags stripped; JS rendered as plain text; no intact version in git history). Rebuilt from the surviving JS with same inputs/defaults/model; added live recalc, on-load defaults, cost-per-worked-hour line, assumptions note; standardized page shell. Math verified in Node with a DOM stub

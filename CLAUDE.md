...
### Session: 2026-08-27 — Foundation Order Sheet, replaces the foundation checklist (PR #250)
- **Purpose the user stated**: a design document that carries a foundation across three audiences. Sales designs the foundation without missing critical details, the estimator prices from it, and once approved it goes to production as the build spec
- **New file** `foundation-order-sheet.html`, two letter pages. Page 1 is the spec in two balanced columns (Foundation Type & Structure, Slab & Under-Slab, Drainage & Radon, Damproofing & Insulation, Openings & Access, Layout & Coordination). Page 2 is Special Reinforcement notes, Permit Inspections (footing / compaction / backfill / brick), a 4.2in sketch box, and four sign-off lines (designed by sales, priced by estimator, approved by client, released to production)
- **`foundation-checklist.html` is now a redirect stub** pointing at the new sheet; links updated on `foundations.html` and `resources.html`. The old checklist's reminders were converted into design-time fields rather than dropped: brick ledge, sump discharge, downspouts, penetrations, top-of-wall elevation and what it matches, anchor bolt coordination, backfill inspection
- **R-value research the user asked for (red-flagged, pending approval)**: SE Michigan is climate zone 5. Basement wall and crawl space wall both R-15 continuous exterior or R-19 cavity interior; slab edge R-10 down 2 ft (2015 MRC / IECC Table R402.1.2, verified via PNNL Building America). Type IX EPS is the below-grade board at about R-4.2 per inch (2"=R-8.4, 3"=R-12.6, 4"=R-16.8), so **4" EPS on walls, 3" at the slab edge**
- **Added beyond the ask, flagged to the user**: the sketch box and the sign-off lines. A foundation design is hard to convey without a footprint sketch, and the sign-off makes the three-audience handoff explicit on the artifact itself
- **CRITICAL CSS BUG FOUND, affects every form on this site**: the `@media print` block is conventionally placed FIRST in each form's `<style>`, before the unmediated "screen" rules. Unmediated rules apply to **all** media, so at equal specificity the later ones **override the print block** and the print tuning is silently inert. This sheet printed at screen sizing and ran 3 pages until the print block was moved to the END of the `<style>`. Fix in this file: print block last, with a comment saying why. **The other forms still have the original ordering**; they have not visibly broken because they are short enough to fit anyway, but their print tuning is equally inert and any future one-page work on them should move the block last first
- **Print-measurement technique, corrected and extended** (supersedes the earlier note): to measure real print heights, copy the file, convert its `@media print {` to `@media screen {`, and **append that converted block at the END of the `<style>`** so it actually wins; then inject a small script that writes `getBoundingClientRect().height` values into a `<div id="M">` and read them with headless Chromium `--dump-dom | grep 'id="M"'`. This gives exact per-element inches instead of pass/fail page counts. Verify the PDF page count with `len(re.findall(rb'/Type\s*/Page[^s]', pdf))`, not the first `/Count`
...
### Session: 2026-09-02 (cont.) — EPA link rot, neighbor letter parent, job start consolidation (PRs #259, #260, #261)
- **PR #259, dead EPA links on `hazardous-materials.html`**: EPA moved files off `/sites/default/files/documents/`. The pre-renovation form and sample checklist both returned **403 with an S3 AccessDenied body**; the Renovate Right brochure on the identical path still resolved, which is how the rot was distinguished from bot-blocking. New locations are under `/system/files/documents/2026-08/` with **expiry dates in the filename** (`exp-4-30-2029`), so EPA re-versions them and these will rot again. Added `epa.gov/lead/renovation-repair-and-painting-program-resources` as the durable index to recover from, and switched the brochure to its landing page `epa.gov/lead/renovate-right`
- **Link-checking note**: prior review passes only validated internal `*.html` links. External links were never checked, which is why this sat broken. A full external sweep found 144 distinct external URLs site-wide; these three were the only EPA document links
- **PR #260, `neighbor-letter.html` parent was wrong**: nav highlighted Demolition and both breadcrumbs pointed there, but the letter is a general "we are working in your neighborhood" introduction handed out at job start, and it was already linked from `pre-construction.html`. Repointed to **Site Setup**. Audited all 32 standalone printables comparing active nav item against breadcrumb parent; this was the only mismatch. `resources.html` still indexes it under the Demolition heading, left alone deliberately
- **PR #261, three checklists into two**: user's framing was keep the protection sheet separate, combine the other two, split by responsibility. New **`job-start-checklist.html`**: Project Manager column (ordered, issued, delivered) and Field/Lead column (loaded and set up on site)
- **The overlap that justified it**: floor protection, plastic and tape were on **all three** sheets; permit box, jobsite sign / yard sign, and lockbox were on two; zip poles, magnet doors, 3/6-mil poly and blue/white tape were on two. The site setup sheet also listed **"Appliance specs" and "Appliance specifications" as separate lines** and carried "Job Start Materials sheet" as a checklist item pointing at the other sheet
- **Key move**: protection materials are deliberately **not** on the job start sheet. A callout points at the Jobsite Protection Checklist, which owns them plus the order protection goes in. That is what breaks the three-way duplication, so every item now lives exactly one place
- **Verification technique worth reusing for any merge**: scripted diff of the union of the old sheets' `<li>` items against the new sheet, with an explicit allowlist of items intentionally relocated. 54 lines became 36 with nothing dropped; the ten not appearing verbatim were all renames or merges
- **`site-set-up-checklist.html` and `job-start-materials-checklist.html` are now redirect stubs**; links collapsed from two entries to one on `pre-construction.html`, `jobsite-protection.html`, and `resources.html`
- **Branches**: `claude/fix-epa-resource-links`, `claude/neighbor-letter-nav`, `claude/consolidate-job-start-checklists`

...
### Session: 2026-08-27 (cont.) — Foundation Order Sheet review pass (PR #252)
- **User's field corrections applied**: footing size gained a linear feet field (estimator needs LF to price); brick ledge drop/width row dropped; downspout extensions gained Qty and LF; "Header required" moved to its own line; anchor bolts/hold-downs, window wells, underground drain design and site constraints all removed from Layout & Coordination; the intro paragraph and the inspection explanation line removed
- **New checkboxes added per user**: Tight Site Conditions, Pump Truck Needed, Slinger Truck Needed (these live in Layout & Coordination)
- **RADON PREP CORRECTION (important)**: on this sheet, "Radon Prep Only" is **a 3" PVC tee stub under the slab. No crock, no pump.** The original draft said drain tile to a crock, which was wrong. Note this does **not** contradict the PR #234 field lesson on `foundations.html`: that entry describes a specific job where a crock *was* installed for radon prep and failed the compaction inspection over plumbing review. The job account stays; the standard spec is the tee stub. Do not "reconcile" one to the other
- **Red flag removed with the rewritten line**: the plumbing sign-off caution rode on the old crock-based radon text and came off with it. The sump pit now only exists under the Full Drainage Package. Told the user it can move to that line if wanted; no answer yet
- **R-value question answered**: 2015 IECC / MRC Table R402.1.2 lists climate zone 5 basement wall and crawl space wall **identically at 15/19**. Added the qualifier that the crawl figure applies to an **unvented** crawl (wall is the insulated plane); a vented crawl insulates the floor above instead. Reference box now reads "Basement and unvented crawl walls"
- **EPS thickness math removed** from the reference box per user; it now carries only the code requirement and "Type IX EPS ~R-4.2/in"
- **Sign-off section removed** per user. Of the two things added beyond the original ask, the **sketch box was kept and the sign-off was cut**. Useful signal: this user wants the drawing space, not process ceremony, on a field artifact
- **Still 2 print pages**: page 1 7.63in, page 2 7.07in of the 10in printable area
- **Branch**: `claude/foundation-sheet-revisions`

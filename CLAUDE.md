...
- **Done this session**:
  ...
  - **Deck**: applied the PR #202 SKU corrections that had never actually been written to the file (LUS212Z->HUS212, LSCA28->LSSR28Z, CBE44/66->ABU44Z/66Z, LSCA212->HUS212, HHUSS212->LSCZ); fixed angle-bracket leg labels (A34 = 2½", A35 = 4½"; old 3"/4"/5" wrong, web-verified). GRK RSS = 100/box (ships by count not by lb, verified). HeadLok = 50/pack (user-confirmed). Clip/joist-tape counts -> generic order reminders (user uses different clip kinds; supplier scales). All deck red flags cleared
...
- **Still pending (mechanical, no input needed, offered next)**: rename roofing-order-sheet-2 -> roofing-order-sheet + redirect; remove `<strong>` bold labels on wood-lvp + tile-order; redirect tile-order orphan to prep/finish; reformat gutter + concrete price sheets to the form template (USER DECISION: keep as dated examples, NO price changes, just template + footer year)

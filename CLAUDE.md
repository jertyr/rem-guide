...
### Session: 2026-08-09 — New Painting page and worksheet (PR #236)
- **New page** `painting.html`, built from the user's page brief notes. Interior only (exterior painting explicitly out of scope per user). Sections: Quick Reference, Scope: What to Specify (surface-by-surface / room-by-room, what a complete scope lists, coat count and sheen per surface), Prerequisites, Process (Phase 1 prime before flooring, Phase 2 finish coats after trim, stain grade trim sequence, doors sent off site), Protection and Prep, Best Practices, Sign-Off and Touch-Up, Client Communication, Resources
- **User decision on structure**: the "Recurring Field Issues" section from the brief was folded into Best Practices and Client Communication rather than kept as its own section, matching the site's positive-framing convention (issues become the practice that prevents them)
- **User decision on Dexter Builders**: explicitly no company-specific split. The Remodelers Guide is not a Dexter Builders product and should not reflect that the user works there; page reads as general pro guidance, addressed to people who already know the trade (reminders and tips, not teaching the trade)
- **Researched/red-flagged additions** (web-verified before adding, pending approval): PCA Standard P1 (definition of a "properly painted surface," 39in/1m viewing distance under finished lighting; touch-up vs. damage-by-others as separate obligations), PCA Standard P4 (GC/contracting entity accepts the substrate as ready to paint, not the painter), GA-214 drywall finish Level 4 vs Level 5 (Level 5 required under gloss/semi-gloss sheen or critical/raking light), PVA primer requires a topcoat to perform, 50°F minimum interior latex application temperature (check product data sheet), pre-stain conditioner timing on softwoods/maple
- **New printable** `painting-worksheet.html`: Room / Element / Color / Sheen / Brand columns, Color widest (34%), all-blank rows per user preference (not pre-filled rooms or elements), notes section, verified to one print page via headless Chromium `--print-to-pdf` page count. Linked from Resources (Interior Finishes) and from painting.html Resources
- **Nav**: "Painting" added after "Countertops" (last content page before Stairs) across all 66 files that carry the sidebar; verified no duplicates and no missed sidebar-carrying pages
- **Cross-links added**: Drywall, Flooring, Tile, Countertops, and Exterior Doors Resources sections each got a line pointing to Painting (priming handoff, phase sequencing, countertop protection, exterior door scope line respectively)

...
### Session: 2026-08-14 — Interior Trim page + Interior Door Order Form (PR #241)
...
- **New page** `interior-trim.html`: standard format (intro, Quick Reference, Prerequisites, Materials and Components, Process, Best Practices, Inspections, Client Communication, Resources). Covers prehung/slab/bifold/bypass/pocket/barn doors, casing, base, window stool and apron, paint grade vs stain grade. Cross-links Drywall, Flooring, Tile, Painting, Stairs for the sequencing handoffs
- **Nav placement**: "Interior Trim" inserted after "Painting", before "Stairs", in all 66 sidebar-carrying files. Reasoning: groups the finish carpentry (Interior Trim + Stairs) and sits next to Painting, its main coordination partner. Orphans, redirect stubs, and standalone printables correctly skipped (14 files)
...
- **DESIGN DECISION (flagged to user, may get reverted)**: built as a **door schedule**, not one-sheet-per-opening like the exterior door form. A house shares one door spec across 10 to 15 openings, so the form is job-wide specs in a 3-column grid (Door / Jamb & Casing / Hardware & Hinges) over a 14-row schedule (Location, Size, Type, Hand, Jamb, Hardware, Notes), plus a legend line and notes box. Told the user it can be swapped to the exterior-style one-door-per-sheet layout with a draw box if preferred
- **Red flags pending approval**: page has door thickness/height/width ranges, jamb widths, RO rule of thumb, hinge fastening, bore/backset, 3/16" casing reveal, nail gauges, acclimation, and the garage-to-house door code items. Form has heights, thickness, jamb widths, bore, backset, hinge size/count, and the hand convention
...
- **Form measured**: content is between 9.2in and 9.4in of the 10in printable area (letter portrait, 0.5in margins), so one page with room to spare. Print notes-box trimmed 0.7in to 0.5in to buy that margin
- **Branch**: `claude/interior-trim-page-and-door-form`
